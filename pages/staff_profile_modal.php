...
if ($userId <= 0) {
    ...
    echo '<div class="alert alert-warning rounded-4 mb-0">ไม่พบข้อมูลเจ้าหน้าที่ที่ต้องการ</div>';
    exit;
}
...
if (!app_can('can_view_all_staff') && !in_array((int) ($user['department_id'] ?? 0), $scope['ids'], true)) {
    ...
    echo '<div class="alert alert-danger rounded-4 mb-0">ไม่มีสิทธิ์ดูข้อมูลเจ้าหน้าที่รายนี้</div>';
    exit;
}
