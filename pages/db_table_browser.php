...
<main class="dash-main db-table-page-main">
    ...
    <div class="db-table-dashboard-frame">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="db-table-row-actions">
                                            <?php if (!empty($tableConfig['edit_allowed'])): ?>
                                                <a href="db_table_browser.php?table=<?= urlencode($tableName) ?>#dbSelectedPanel"
                                                   class="db-table-row-btn is-edit"
                                                   title="เลือกตารางและดูข้อมูล">
                                                    <i class="bi bi-pencil-square"></i> แก้ไข
                                                </a>
                                            <?php else: ?>
                                                <a href="db_table_browser.php?table=<?= urlencode($tableName) ?>#dbSelectedPanel"
                                                   class="db-table-row-btn is-readonly"
                                                   title="ตารางนี้เป็นอ่านอย่างเดียว">
        ...
        <?php if ($config): ?>
            <section class="db-table-selected-panel" id="dbSelectedPanel">
                <div class="db-table-selected-toolbar">
                    <div class="db-table-selected-info">
                        <span class="db-table-selected-eyebrow">Selected Table</span>
                        <h2>
                            <?= htmlspecialchars($config['label']) ?>
                            <?php if (empty($config['edit_allowed'])): ?>
                                <span class="db-table-readonly-badge"><i class="bi bi-lock-fill"></i> อ่านอย่างเดียว</span>
                            <?php endif; ?>
                        </h2>
                    ...
                    <div class="db-table-selected-actions">
                        ...
                        <div class="db-table-selected-export">
                            <a href="report_print.php?<?= htmlspecialchars($printQuery) ?>"
                               target="_blank" rel="noopener"
                               class="db-table-export-btn"
                               title="พิมพ์รายงาน <?= htmlspecialchars($config['label']) ?>">
                                <i class="bi bi-printer"></i>
                                <span>พิมพ์รายงาน</span>
                            </a>
                            <a href="report_print.php?<?= htmlspecialchars($pdfQuery) ?>"
                               target="_blank" rel="noopener"
                               class="db-table-export-btn"
                               title="ส่งออก PDF <?= htmlspecialchars($config['label']) ?>">
                                <i class="bi bi-filetype-pdf"></i>
                                <span>ส่งออก PDF</span>
                            </a>
                            <a href="export_report.php?<?= htmlspecialchars($csvQuery) ?>"
                               class="db-table-export-btn"
                               title="ส่งออก CSV <?= htmlspecialchars($config['label']) ?>">
...
<div id="userEditModal" class="db-user-modal-overlay" hidden aria-hidden="true">
    <div class="db-user-modal-card" role="dialog" aria-modal="true" aria-labelledby="userEditModalTitle">
        <div class="db-user-modal-header">
            <div>
                <p class="db-user-modal-eyebrow" id="userEditModalEyebrow">Admin Only</p>
                <h2 id="userEditModalTitle">แก้ไขข้อมูล</h2>
                <p id="userEditModalDesc">แก้ไขข้อมูลและบันทึก ระบบจะบันทึก audit log ทุกครั้งที่มีการเปลี่ยนแปลง</p>
            </div>
        ...
        <div class="db-user-modal-body">
            <div class="db-user-modal-loading" id="userEditModalLoading">
                ...
                <span id="userEditModalLoadingText">กำลังโหลดข้อมูล...</span>
            </div>
            <iframe
...
(function () {
    ...
    const modalDesc    = document.getElementById('userEditModalDesc');
    const modalEyebrow = document.getElementById('userEditModalEyebrow');
    const loadingText  = document.getElementById('userEditModalLoadingText');

    let lastTrigger = null;
    const body = document.body;

    function openUserEditModal(url, trigger, meta) {
        if (!url) return;
        const frameUrl = new URL(url, document.baseURI || window.location.href);
        frameUrl.searchParams.set('modal', '1');
        lastTrigger = trigger || null;

        // ปรับ title / desc ตาม table ที่กด
        if (meta) {
            if (modalTitle)   modalTitle.textContent   = meta.title   || 'แก้ไขข้อมูล';
            if (modalDesc)    modalDesc.textContent    = meta.desc    || 'ระบบจะบันทึก audit log ทุกครั้งที่มีการเปลี่ยนแปลง';
            if (modalEyebrow) modalEyebrow.textContent = meta.eyebrow || 'Admin Only';
            if (loadingText)  loadingText.textContent  = 'กำลังโหลด ' + (meta.title || 'ข้อมูล') + '...';
        }
